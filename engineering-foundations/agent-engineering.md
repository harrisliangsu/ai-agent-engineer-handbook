...
**工程权衡**：
- **硬上限要按任务类别分档，不是全局常量**：长 refactor 与一次性问答的合理上限差一个数量级，全局取统一值要么过早掐断、要么形同虚设。生产里通常按 task type / tool risk 各配一套阈值，并把"接近上限"作为告警事件而非静默截断。
- **重复检测窗口 N 不能贪小**：N=2 在合法的轮询、分页、反复 `read_file` 场景下误报极高；实战中需要把"相同 error"也纳入指纹，并对只读类工具放宽阈值，否则 escape hatch 会被误触发，反而让 agent 提前放弃可完成的任务。
- **分层 compute 是这套防御里 ROI 最高的一条**：把规划 / 反思留给强模型，把工具参数填充、格式化、retry 决策下放给便宜模型，社区与厂商 cookbook 普遍报告 token 成本 **降 ~70%**——同时也意味着循环烧掉的钱更少，硬上限的"血条"更长。
- **这套防御不能挡住 specification 层面的死循环**：当目标本身就是不可达的（编译不通过的代码、被对方 API 拒绝的请求），agent 即使每步都换不同方法也会在"语义同构"的尝试间打转。此时只能靠上层 verifier / HITL gate 兜底，参见 §9.4 与第 10 节。
