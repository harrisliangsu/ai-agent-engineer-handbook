...
关键转折：**生产级 LLM 应用的核心难度从"写 prompt"迁移到了"管 context"**——这是 Agent（[03](./03_agent_engineering.md)）和驾驭工程（[04](./04_harness_engineering.md)）的前提。
...
回到 [01_prompt_engineering.md](./01_prompt_engineering.md) §1.1：标准 attention 是 N² 复杂度。上下文从 32K 翻到 128K，注意力计算量是 16 倍而非 4 倍。对推理服务来说这意味着两件事：
...
每个 attention 层在推理时要维护 K 和 V 的缓存（避免重复计算）。**一个 70B 模型在 128K 上下文时，KV cache 占用可达 40+ GB，比模型权重本身还大**。这就是为什么 vLLM 的 PagedAttention 和 SGLang 的 RadixAttention 这类"管 KV cache"的技术成为推理服务器的核心竞争力（详见 [04_harness_engineering.md](./04_harness_engineering.md) §3.9.2）。
...
- **Step-Back Prompting**：先把 query 抽象成更宏观的问题，用抽象问题的检索结果做铺垫
...
实证：Self-RAG 在生产部署中能减少 25-40% 的"不必要检索"——因为模型学会了对自己已有知识的事情不去检索。

详见 [03_agent_engineering.md](./03_agent_engineering.md) §2 范式演进、§5 框架对比。
...
工具集成在 Agent 实现层面的细节 → [03_agent_engineering.md](./03_agent_engineering.md) §3.1 与 §5；
LLM 网关 / observability / 推理服务器层面的基础设施 → [04_harness_engineering.md](./04_harness_engineering.md) §3.9 附录（注：04 章把这 4 类显式标记为"相关但非 harness 工程"）。
...
- 想理解长上下文背后的 Transformer 物理 → [01_prompt_engineering.md](./01_prompt_engineering.md) §1.1
- 想理解 Agentic RAG / 记忆 在 Agent 里怎么用 → [03_agent_engineering.md](./03_agent_engineering.md) §3 + §4
- 想理解 KV cache / prefix caching 推理服务器层面 → [04_harness_engineering.md](./04_harness_engineering.md) §3.9.2
