...
<details id="g-biz-metrics">
<summary><strong>GMV / UV / CSAT / NPS / 转化率</strong> <span class="term-tag">业务指标</span> — 电商 / 旅行 / 客服业务最常用的几个北极星指标</summary>

业务侧验收 Agent 是否上线最看的几个指标：

- **GMV (Gross Merchandise Value)**：商品交易总额，电商核心指标。日均 GMV、月 GMV、增量 GMV（A/B 实验组比对照组多出来的）都是常见口径。
- **UV (Unique Visitor) / DAU (Daily Active User)**：独立访客数 / 日活用户。UV 看流量，DAU 看粘性。
- **PV (Page View)**：页面浏览量，UV × 人均浏览次数。
- **CTR (Click-Through Rate)**：点击率，曝光后点击占比；推荐系统核心指标。
- **转化率 (Conversion Rate)**：漏斗各环节转化——列表页→详情页、详情页→订单页、详情页→支付。电商 / 旅行场景 +1pp 即显著、+1.5-2pp 算大幅提升。
- **CSAT (Customer Satisfaction Score)**：用户满意度评分（1-5 或 1-10），客服 Agent 必看。
- **NPS (Net Promoter Score)**：净推荐值，"你有多大可能推荐给朋友"，-100 到 100。
- **首轮解决率 / FCR (First Contact Resolution)**：首次交互就解决问题的比例，客服 Agent 关键指标。

**Agent 工程关注**：把模型 / prompt / RAG 改动放进 A/B 看这些业务指标，而非只看离线 benchmark 分数。改动让 benchmark 涨但 GMV / CSAT 跌的情况非常常见。
...
<details id="g-yarn">
<summary><strong>NTK-aware scaling / YaRN</strong> <span class="term-tag">架构</span> — RoPE 长度外推方案，让 2k 训练的模型能跑 32k / 128k context</summary>

[RoPE](#g-rope) 纯外推会崩——高频分量在训练上下文外旋转角度落入未见区域。两种工程化方案：

- **NTK-aware scaling**（NTK = Neural Tangent Kernel）：对低频维度做插值（拉伸 base），高频维度近乎不变。简单有效，不需重训。
- **YaRN**（Peng et al. 2023）：在 NTK 基础上加 **attention temperature** 调整 + 按频率维度**分段缩放**（高频严格保留、低频强插值），需少量长上下文数据微调（万级 token 长度样本）。

**主流应用**：

- Llama-3.1 128k → YaRN
- Qwen2.5 / Qwen3 → YaRN
- Mistral-7B 32k 早期版本 → NTK
- GPT-4 / Claude → 闭源，可能用自研方案 + 长上下文持续训练

**经验数字**：YaRN 在 4× 上下文扩展（2k→8k 或 32k→128k）几乎无损；8× 以上仍有性能下降。极致长上下文（1M+）通常需要重训而非仅靠 YaRN。
...
<details id="g-embed-models">
<summary><strong>主流 embedding 模型选型</strong> <span class="term-tag">RAG</span> — BGE-M3 / Cohere v4 / Voyage-3 / OpenAI text-3-large / Qwen3-Embedding 怎么选</summary>

2026 主流 embedding 模型对比：

- **OpenAI text-embedding-3-large**（3072 维）：英文 baseline，API 友好，$0.13/M tokens；中文一般。
- **Cohere embed-v4**（1024-1536 维，多模态）：MTEB v2 顶级，**支持文本+图像同空间**，多语言 100+；企业 SaaS 首选。
- **Voyage-3-large**（1024 维）：MTEB v2 综合第一，中文表现优秀，retrieval 任务略胜 Cohere。
- **BGE-M3**（BAAI，1024 维）：**开源 SOTA**，**同时支持 dense + sparse + multi-vector (ColBERT-style)** 三种模式，中英双语；自托管首选。
- **Qwen3-Embedding-8B**（阿里 2025）：开源中文 SOTA 之一，与 BGE-M3 各有胜场。
- **Conan-Embedding**（腾讯）/ **Yi-Embedding**（零一）：国产中文方案。

**选型框架**：

1. 中文为主 → BGE-M3 / Voyage-3 / Qwen3-Embedding
2. 多模态 → Cohere v4
3. 纯英文 + API → OpenAI text-3-large
4. 自托管私有部署 → BGE-M3
...
- MTEB 有版本差异（v1 / v2 / MMTEB），不同模型挂的版本不一样，**直接横比误导**——自家场景必须重测
- Dense vs Sparse 看场景：精确词命中（专有名词 / 编号）BM25 + dense 混合（[hybrid search](#g-hybrid-search)）比单 dense 提升 5-15pp

</details>

<details id="g-vector-db">
<summary><strong>vector DB / 向量数据库</strong> <span class="term-tag">RAG</span> — 专门存 embedding 并支持近似最近邻 (ANN) 检索；Pinecone / Milvus / Qdrant 等</summary>
...
<details id="g-rag-variants">
<summary><strong>GraphRAG / Self-RAG / CRAG / Agentic RAG</strong> <span class="term-tag">RAG</span> — 朴素 RAG 的 4 个进阶变体，各解决不同短板</summary>

朴素 [RAG](#g-rag) 在多文档跨实体推理、召回质量不稳、多跳查询场景下都会力不从心。四个常见进阶方案：

1. **GraphRAG**（微软 2024）：先用 LLM 从文档抽 entity + relation 建知识图谱，按社区做层次摘要；query 时既能 vector search 又能图遍历。**解决"跨多文档跨实体推理时朴素 RAG 召回不全"**。代价：ingestion 成本大涨（每 chunk 一次 LLM 抽取）。
2. **Self-RAG**（Asai 2023）：训练单一 LM 自己生成 reflection token 决定 (a) 要不要检索 (b) retrieved 是否相关 (c) answer 是否被 context 支撑 (d) 整体 utility。**解决"什么时候检索、检索后怎么用"的判断**。需训练。
3. **CRAG (Corrective RAG，Yan 2024)**：retrieved chunks 经 lightweight evaluator 打分，分高直接用、中等触发 web 重新搜索、低退回基础生成。**解决"召回质量参差不齐"时盲目使用 retrieved**。
4. **Agentic RAG**：让 agent 决定 query rewriting → 多轮检索 → 子 agent 分工。**解决多跳问题、需迭代检索**。DeepResearch 类产品就是这一路。

**实战选择**：

- 普通 QA → 朴素 RAG + [reranker](#g-reranker)
- 跨文档跨实体复杂推理 → GraphRAG
- 召回质量不稳 → CRAG
- 多跳推理 / 需迭代搜索 → Agentic RAG
...
<details id="g-langchain">
<summary><strong>LangChain / LangGraph / LlamaIndex / Haystack</strong> <span class="term-tag">Agent</span> — 主流 Agent / LLM 应用开发框架</summary>

2024-2026 主流框架对比：

- **LangChain**（Harrison Chase / LangChain Inc.）：最早出圈的 LLM 应用框架，定位是"LLM 应用的胶水"——连 LLM、prompt 模板、工具、向量库、文档加载器。**优点**：生态最全；**缺点**：抽象层多容易过度封装，Anthropic *Building Effective Agents* 明确反对"过度抽象"。
- **LangGraph**（LangChain Inc.）：基于 LangChain 的**有状态 graph 编排**——把 agent / workflow 建模成节点 + 边，每个节点是 LLM 调用或工具，边可条件跳转。比 LangChain 更适合复杂多步 / 多 agent 协同；阿里飞猪等多 agent 业务系统主流选择之一。
- **LlamaIndex**（Jerry Liu）：聚焦 RAG，文档摄取 / 索引 / 检索 / 查询 pipeline 设计精细。**RAG 重场景比 LangChain 更专业**。
- **Haystack**（deepset）：开源 RAG 框架，企业级（含 K8s 部署 / API server），欧洲社区主导。
- **Anthropic SDK 直调**：Anthropic 在 *Building Effective Agents* 推荐——简单场景直接调 API 比裹一层 framework 更易理解、bug 更少。

**经验**：先用 SDK 直调写原型；workflow 模式（routing / parallelization / orchestrator-workers）成熟后再考虑 LangGraph；纯 RAG 场景考虑 LlamaIndex；避免一上来就 LangChain "全家桶"。
...
<details id="g-tracing-tools">
<summary><strong>Trace + Eval 工具栈</strong> <span class="term-tag">评估</span> — Langfuse / LangSmith / Phoenix / Helicone / Patronus 等可观测平台</summary>

LLM 应用 / Agent 的"全链路追踪 + 评估"工具，2026 主流：

- **Langfuse**（开源 + SaaS）：开源 SOTA，self-hostable，trace + prompt 版本化 + eval + cost 跟踪一站式。中小团队首选。
- **LangSmith**（LangChain Inc.，闭源 SaaS）：LangChain 生态原生集成最深，trace 可视化好，eval framework 成熟。
- **Phoenix**（Arize AI 开源）：聚焦 LLM 应用的 trace + monitoring，集成 OpenTelemetry 好。
- **Helicone**（开源 + SaaS）：proxy 模式接入快，cost / cache hit / latency 监控直观。
- **Patronus**（SaaS）：聚焦"hallucination / safety detector"，做生产级实时拦截，企业方向。
- **Datadog / Grafana / Prometheus**：通用 APM / metrics 平台也开始支持 LLM trace（Datadog LLM Observability、Grafana Tempo）。

**核心能力对齐项**：

1. **Trace**：prompt → tool_call → response → cost / latency 串成完整 trace
2. **Prompt 版本化**：每次 prompt 改动一个 hash，可在 dashboard 切换并对比
3. **Eval**：CI 上跑黄金集，回归报警
4. **Cost & Cache hit rate**：单 trace 成本、cache 命中分布
5. **User feedback / thumb up-down**：直接打到 trace 上做 bad case 入库

**选型**：自建优先 Langfuse；LangChain 重度用户 LangSmith；纯 trace 监控用 Phoenix；安全 / 实时拦截用 Patronus。

</details>

<details id="g-llm-benchmarks">
<summary><strong>主流 LLM eval benchmark</strong> <span class="term-tag">评估</span> — MMLU / MT-Bench / Chatbot Arena / HumanEval / LiveBench / BFCL 各测什么</summary>

按测什么能力分类：

- **MMLU (Massive Multitask Language Understanding)**：57 学科多选题，**通用知识**基准；GPT-4 ~88%。批评：题目泄漏严重、单选难辨细节差异。
- **HumanEval**（OpenAI 2021）：164 道 Python 编程题 + 单测验证，**代码生成**基准。**已被刷穿**（多数前沿模型 90%+），不再有区分度，推荐 SWE-bench Verified。
- **MT-Bench**（LMSYS 2023）：80 道多轮对话题，用 GPT-4 当 judge 1-10 打分。**对话能力**经典 benchmark，但 [judge bias](#g-llm-judge) 严重。
- **Chatbot Arena / LMArena**（LMSYS / Berkeley）：**真人盲评 pairwise**，Elo 评分。最权威也最贵；2024 起被怀疑刷分（厂商针对性优化"问候 / 解释 / 列表格式"等 Arena 偏好）。
- **LiveBench**（Abacus AI 等）：**月度更新**题目防数据污染，coverage 推理 / 数学 / 代码 / 语言 / 数据 / 指令；持续更新性最强。
- **BFCL (Berkeley Function-Calling Leaderboard)**：**Tool use / function calling** 标准 benchmark，含 simple / multiple / parallel / multi-turn。
- **GSM8K / MATH / AIME**：**数学推理**基准，AIME 是 R1 / o1 当前主战场。

**实战**：从 2026 视角，**单一 benchmark 都已不可信**——必须多 benchmark 交叉验证 + 自家黄金集 + Arena 真人盲评三层。具体见 [E11](#e11.-agent-benchmark-怎么被-hack)、[H3](#h3.-benchmark-是怎么被-hack)。

</details>

<details id="g-eval">
<summary><strong>Eval / Evaluation</strong> <span class="term-tag">评估</span> — LLM 应用 / Agent 的"打分"环节：offline / online / CI 三层评估</summary>

LLM / Agent 应用的 eval 不是单一动作，是一套体系。常见层次：

- **Offline eval**：在固定 dataset / 黄金集上跑模型 / prompt / pipeline，看指标。每次 prompt 改动跑回归，CI 上自动化。
- **Online eval**：线上流量上抽样评估，配合 A/B 看业务 metric 变化、bad case 检测、用户反馈打标。
- **Pre-prod eval**：上线前在预发流量上做 shadow eval（双跑不返回用户），对比新旧版本轨迹差异。
- **Adversarial eval / Red team**：人工 + 自动化构造对抗样本（jailbreak、prompt injection、edge case），评估安全护栏。

**Eval 工具栈分类**：

1. **通用 framework**：OpenAI Evals、DeepEval、Promptfoo、Inspect AI——CLI / 库形式跑评估
2. **集成在可观测平台**：Langfuse / LangSmith / Phoenix 都内置 eval framework，与 trace 联动
3. **RAG 专用**：[RAGAS](#g-ragas)（faithfulness / context relevancy 等 4 维）
4. **Tool use 专用**：BFCL（Berkeley Function-Calling Leaderboard）
5. **领域专用**：每家自家黄金集 + LLM-judge rubric

**Eval 类型按打分方式**：

- **Reference-based**（有标准答案）：精确匹配、BLEU / ROUGE、单测通过、答案集合命中——客观但需标注
- **Reference-free**（无标准答案）：[LLM-as-judge](#g-llm-judge) 给 0-1 分或 1-5 分；适合开放生成、对话；judge bias 是大坑
- **Pairwise**：A vs B 对比，模型选型常用，agreement 比 pointwise 高
- **User feedback**：thumb up/down、CSAT、留存——业务侧最终验收

**经验法则**：(1) 没 eval 就改 prompt = 改盲；(2) Eval model 必须 ≥ 被评估 model（判官原则）；(3) 黄金集要版本化 + 私有；(4) 每个 bad case 都要回归进黄金集——harness engineering 闭环。
